        ...
        do {
            ...
            switch (choice) {
                ...
                case 4: // Book Flight
                    ...
                    long flightNum = input.nextLong();
                    Flight selectedFlight = null;
                    for (Flight f : Flight.Flights) {
                    ...
                    if (selectedFlight != null) {
                        ...
                        for (int i = 0; i < passengerCount; i++) {
                            System.out.println("Passenger " + (i+1) + " name:");
                            String name = input.nextLine();
                            System.out.println("Passport number:");
                            String passport = input.nextLine();
                            System.out.println("Date of birth (YYYY-MM-DD):");
                            String dob = input.nextLine();
                            passengers.add(new Passenger(name, passport, dob));
                        }
                        Booking newBooking = ((Customer) user).createBooking(selectedFlight, passengers);
                        System.out.println("Booking created! Reference: " + newBooking.getBookingReference());
                    }
                    break;

                case 5: // Complete Payment
                    System.out.println("Enter booking reference:");
                    String ref = input.next();
                    Booking bookingToPay = null;
                    for (Booking b : AirPort.Bookings) {
                    ...
                    if (bookingToPay != null) {
                        System.out.println("Amount due: " + bookingToPay.getFlight().getPrice());
                        System.out.println("Payment method (Credit Card/Bank Transfer):");
                        String method = input.next();
                        Payment payment = new Payment(bookingToPay.getFlight().getPrice(), method);
                        payment.processPayment();
                        bookingToPay.confirmBooking(payment);
                        System.out.println("Payment completed! Status: " + payment.getStatus());
                    }
                    break;
                case 6:
                    System.out.println("\nBOOKING HISTORY\n");
